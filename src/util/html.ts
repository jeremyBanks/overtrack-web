...
// values, producing an HTML object. This can be converted to a string using the
// .toString() method. If you need to interpolate a dynamic HTML value, wrap it
// with HTML.UnsafeRaw(...). Arrays of strings will be concatenated.
//
...
const HTMLRuntimeType = class HTML {
...
const htmlTag = (strings: TemplateStringsArray, ...substitutions: any[]): HTML => {
  const escapedFlattenedSubstitutions =
    substitutions.map(s => ([] as any[]).concat(s).map(HTML.escape).join(''));
  const pieces = [];
...
const stringTag = (strings: TemplateStringsArray, ...substitutions: any[]): string => {
  return HTML(strings, ...substitutions).toString();
};

// HTML.fragment`...` safely parses the interpolated HTML as an HTMLFragment.
const fragmentTag = (strings: TemplateStringsArray, ...substitutions: any[]): DocumentFragment => {
...
const elementTag = (strings: TemplateStringsArray, ...substitutions: any[]): Element => {
  const html = HTML(strings, ...substitutions);
  const doc = (new DOMParser).parseFromString(`<!doctype html><body><template>${html}</template>`, 'text/html');
